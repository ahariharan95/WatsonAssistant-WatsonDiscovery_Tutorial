...
# Unpacking Functions-- unpacks json objects
def unpack_intents(series,dict_name):
    #Initialize blank 'rows' array
    rows = [] 
    
    # Append row data 
    for items in series.to_list(): 
        data_row = items[dict_name]
        if not data_row:
            rows.append({})
        else:
            for row in data_row: 
    ...
    dict_df.columns = ['intent_confidence','intent_type']
    
    return dict_df

def unpack_context(series,dict_name):
    data = [column.get(dict_name) for column in series]
    context = pd.DataFrame.from_dict(data)
    system_data = pd.DataFrame(context.system.to_list(), columns=['branch_exited','branch_exited_reason','dialog_turn_counter','timezone','assistant_id'])
    metadata_df = pd.DataFrame(context.metadata.to_list())
    
    #Combine dataframes
    joined_df = pd.concat([context,system_data,metadata_df],axis=1)
    
    #Drop unneeded columns
    joined_df.drop(columns=['metadata','system'], inplace=True)
    
    return joined_df

def unpack_nested_dictionary(series,dict_name):
    #Initialize blank 'rows' array
    rows = [] 
...
def unpack_dictionary(series,dict_name):
    data = [column.get(dict_name) for column in series]
